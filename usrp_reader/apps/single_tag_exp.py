...
class my_top_block(gr.top_block):
	...
	mm = rfid.clock_recovery_zc_ff(4,2)
...
#End RX
	...
	command_gate.set_tag_q_out(self.reader.get_tag_q())
        #agc2 = gr.agc2_ff(0.3, 1e-3, 1, 1, 1000) 
...
def main():
    ...
    log_file = open("time_series_"+sys.argv[1]+".log", "w")
    amplitude = int(sys.argv[2])

    tb = my_top_block()

    tb.start()
    while tb.reader.check_reader_state():
	sleep(1)
        '''
...
def print_log_msg(msg, log_file):
    ...
    if msg.type() == LOG_RN16:
	    ...
	    log_file.write(fields[-1] + ",RN16,1," + rn16 + ","+snr + "\n");
        else:
            #print "%s\t    %s RN16: %04X%s" %(fields[-1],fBlue, tmp, fReset)
            #log_file.write(fields[-1] +",RN16,0," + "%04X" % tmp + "," +snr + "\n");
	    print "%s\t    %s RN16: %s%s" %(fields[-1],fBlue, rn16, fReset)
            log_file.write(fields[-1] +",RN16,0," + rn16 + "," +snr + "\n");
