...
rfid_reader_f::rfid_reader_f (int sample_rate)
  : gr_block ("reader_f",
		   ...
		   gr_make_io_signature (1, 1, sizeof (float))),
    d_sample_rate (sample_rate), d_ctrl_q(gr_make_msg_queue(1000)) //100
{
  init_global_reader_state();

  log_q = gr_make_msg_queue(50000000);//Holds log messages, drained by python app
  out_q = gr_make_msg_queue(1000);  //Holds messages for transmission at the end of work
  tag_q = gr_make_msg_queue(1000);
  ...
  memcpy(d_reader_framesync, tmp_framesync, d_reader_framesync_len * sizeof(float));
   ...
   global_reader_state->reader_state = SLEEP_CYCLE_PROBLE;
   for(int i=0; i<global_reader_state->tag_num; i++){
	global_reader_state->tag_sleep_cycle_table[i] = -1;
	global_reader_state->tag_sleep_cycle_runtime[i] = -1;
	global_reader_state->tag_session_id_list[i] = false;
   }

   for(int i=0; i<global_reader_state->tag_num; i++){
	global_reader_state->tag_throughput[i] = 0;
   }

   tag_inventory_count = (int *) malloc(global_reader_state->tag_num*sizeof(int));
   for(int i=0; i<global_reader_state->tag_num; i++){
	tag_inventory_count[i] = 0;
   }
					 ...
					 gr_vector_void_star &output_items)
 {
   ...
   if(ctrl == TIMER_FIRED){
	...
	if(global_reader_state->num_bits_decoded>0){
		...
		sprintf(tmp2, ",%d,%d\n", tag_id_pre+1, global_reader_state->num_bits_decoded);
         	strcat(tmp, tmp2);

         	log_msg(LOG_RN16, tmp, LOG_OKAY);
	}

	global_reader_state->tag_session_id = 0;
        char CMD[5] = "0001";
        memcpy(d_CMD, CMD, 5);
	char CMD_SN[5] = "0000";
        memcpy(d_CMD_sn, CMD_SN, 5);
	
	/*if( global_reader_state->cur_cycle<=global_reader_state->tag_num*global_reader_state->tag_training_round ){
		global_reader_state->tag_session_id = global_reader_state->tag_id_training;
		if(global_reader_state->tag_id_training<15){
			char CMD[5] = "0000";
			CMD[0] = (((global_reader_state->tag_id_training+1) & 0x08) == 0x00 ? '0':'1');
			CMD[1] = (((global_reader_state->tag_id_training+1) & 0x04) == 0x00 ? '0':'1');
			CMD[2] = (((global_reader_state->tag_id_training+1) & 0x02) == 0x00 ? '0':'1');
			CMD[3] = (((global_reader_state->tag_id_training+1) & 0x01) == 0x00 ? '0':'1');
        	        memcpy(d_CMD, CMD, 5);
                	char CMD_SN[5] = "0000";
	                memcpy(d_CMD_sn, CMD_SN, 5);
		}
			...
			memcpy(d_CMD, CMD, 5);
			char CMD_SN[5] = "0000";
                        CMD_SN[0] = (((global_reader_state->tag_id_training-15+1) & 0x08) == 0x00 ? '0':'1');
                        CMD_SN[1] = (((global_reader_state->tag_id_training-15+1) & 0x04) == 0x00 ? '0':'1');
                        CMD_SN[2] = (((global_reader_state->tag_id_training-15+1) & 0x02) == 0x00 ? '0':'1');
                        CMD_SN[3] = (((global_reader_state->tag_id_training-15+1) & 0x01) == 0x00 ? '0':'1');
                        memcpy(d_CMD_sn, CMD_SN, 5);
		}

                global_reader_state->tag_throughput[global_reader_state->tag_id_training] += global_reader_state->num_bits_decoded;
                if( global_reader_state->cur_cycle==(global_reader_state->tag_id_training*global_reader_state->tag_training_round+1) ){
                        time1 = time(0);
                }
                else if( global_reader_state->cur_cycle==(global_reader_state->tag_id_training+1)*global_reader_state->tag_training_round ){
                        time2 = time(0);
                        global_reader_state->tag_throughput[global_reader_state->tag_id_training] /= time2-time1;
			global_reader_state->tag_id_training++;
                }
	}

        if( global_reader_state->cur_cycle == (global_reader_state->tag_num*global_reader_state->tag_training_round) ){
                for(int i=0; i<global_reader_state->tag_num; i++){
                	global_reader_state->tag_sleep_cycle_runtime[i] = global_reader_state->tag_sleep_cycle_table[i];
        	}
        }

	if( global_reader_state->cur_cycle>global_reader_state->tag_num*global_reader_state->tag_training_round ){
		global_reader_state->reader_state = INVENTORY;

		if(global_reader_state->cur_cycle==19999){
			FILE * file;
			file = fopen("tag_probing_results.txt", "w");
			for(int i=0; i<global_reader_state->tag_num; i++){
				fprintf(file, "%LF\t %d\t %d\t %d\n", global_reader_state->tag_throughput[i], tag_inventory_count[i], global_reader_state->tag_sleep_cycle_table[i], global_reader_state->tag_sleep_cycle_runtime[i]);
			}
			fclose(file);
			printf("idle slot counter:%d\n", idle_slot_counter);
		}

		tag_q_msg = tag_q->delete_head_nowait();
		if(!tag_q_msg && tag_ranked_q.empty()){
			if(tag_id_pre != -1){
				printf("%d\n", tag_id_pre);
				global_reader_state->tag_session_id_list[tag_id_pre] = false;
        	                global_reader_state->tag_sleep_cycle_runtime[tag_id_pre] = global_reader_state->tag_sleep_cycle_table[tag_id_pre];
			}

			printf("reader's idle message.\n");
			char CMD[5] = "1111";
                        memcpy(d_CMD, CMD, 5);
			char CMD_SN[5] = "1111";
                        memcpy(d_CMD_sn, CMD_SN, 5);
			...
			if(tag_q_msg){
				int tag_id = tag_q_msg->msg()[0];
				float tag_throughput = global_reader_state->tag_throughput[tag_id];

				tag_ranked_q.push(Tag_Info(tag_id, tag_throughput));
			...
			int tag_id = tag_ranked_q.top().tag_id;
                        tag_ranked_q.pop();
                        tag_id_pre = tag_id;

			tag_inventory_count[tag_id] += 1;

			printf("tag id:%d\n", tag_id);
			
			if(tag_id<15){
	                        char CMD[5] = "0000";
        	                CMD[0] = (((tag_id+1) & 0x08) == 0x00 ? '0':'1');
                	        CMD[1] = (((tag_id+1) & 0x04) == 0x00 ? '0':'1');
                        	CMD[2] = (((tag_id+1) & 0x02) == 0x00 ? '0':'1');
	                        CMD[3] = (((tag_id+1) & 0x01) == 0x00 ? '0':'1');
        	                memcpy(d_CMD, CMD, 5);
                	        char CMD_SN[5] = "0000";
                        	memcpy(d_CMD_sn, CMD_SN, 5);
	                }
                        	...
                        	memcpy(d_CMD, CMD, 5);
	                        char CMD_SN[5] = "0000";
        	                CMD_SN[0] = (((tag_id-15+1) & 0x08) == 0x00 ? '0':'1');
                	        CMD_SN[1] = (((tag_id-15+1) & 0x04) == 0x00 ? '0':'1');
                        	CMD_SN[2] = (((tag_id-15+1) & 0x02) == 0x00 ? '0':'1');
	                        CMD_SN[3] = (((tag_id-15+1) & 0x01) == 0x00 ? '0':'1');
        	                memcpy(d_CMD_sn, CMD_SN, 5);
                	}
		}
   ...
   if(ctrl == BITS_DECODED){
     if(global_reader_state->last_cmd_sent == QUERY || global_reader_state->last_cmd_sent == QREP){
       ...
       /*if (global_reader_state->tag_bit_vector[global_reader_state->num_bits_decoded_last - 1] == '0'){
 
	 global_reader_state->tag_bit_vector[global_reader_state->num_bits_decoded_last] = '\0';
	 char tmp[10000];
	 char tmp2[10000];
	 strcpy(tmp, global_reader_state->tag_bit_vector);
	 sprintf(tmp2, ",%f\n", global_reader_state->std_dev_signal / global_reader_state->std_dev_noise);
...
rfid_reader_f::send_read(){
 ...
 rfid_reader_f::gen_query_cmd(){
   ...
   if(strcmp(d_M, "10") == 0){
     ...
     global_reader_state->tag_zero_cor_vec = m4_data_zero_vec;
