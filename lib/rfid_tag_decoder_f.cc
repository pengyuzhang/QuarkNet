					...
					gr_vector_void_star &output_items)
{
  ...
  double sum = 0, sum_one = 0, sum_zero = 0;
  double total_pwr = 0;
  float score = 0, score_one = 0, score_zero = 0;
  ...
  //for(int i = 0; i < std::min(ninput_items[0], noutput_items); i++){
	...
	sum_one += global_reader_state->tag_one_cor_vec[j] * (in[i + j]);
	...
	avg += in[i + j];
      ...
      float var = 0;
      for(int j = 0; j < global_reader_state->tag_one_cor_vec_len; j++){
	var += pow(in[i + j]-avg,2);
      }
      var = var/global_reader_state->tag_one_cor_vec_len;
      var = sqrt(var);
